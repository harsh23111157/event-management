...
def dashboard_nav_context(request):
    ...
    if user and user.is_authenticated:
        ...
        try:
            ...
            # Notifications: fetch top 5 and count unread in 1 query
            notifs = list(Notification.objects.filter(recipient=user).order_by("-created_at")[:5])
            ctx["recent_notifications"] = notifs
            ctx["unread_notifications_count"] = Notification.objects.filter(recipient=user, is_read=False).count()

            cache_key = f"nav_counts_{user.id}_{user.role}"
            ...
            if cached_counts is not None:
                ...
                if user.is_admin:
                    ...
                    counts_data["sidebar_pending_events"] = p_ev
                    counts_data["sidebar_pending_expenses"] = p_ex
                    counts_data["sidebar_overdue_tasks"] = od_t
                    counts_data["unread_alerts_count"] = p_ev + p_ex + (1 if od_t > 0 else 0)
                    ...
                    counts_data["sidebar_rejected_events"] = rej_ev
                    counts_data["sidebar_overdue_tasks"] = od_t
                    counts_data["unread_alerts_count"] = rej_ev + (1 if od_t > 0 else 0)
                elif user.is_finance:
                    p_ex = Expense.objects.filter(status=ExpenseStatus.PENDING).count()
                    counts_data["sidebar_pending_expenses"] = p_ex
                    counts_data["unread_alerts_count"] = p_ex
                elif user.is_staff_member:
                    my_od = EventTask.objects.filter(assigned_to=user).exclude(status=TaskStatus.COMPLETED).filter(due_date__lt=now).count()
                    counts_data["sidebar_my_overdue"] = my_od
                    counts_data["unread_alerts_count"] = my_od

                cache.set(cache_key, counts_data, timeout=15)
                ctx.update(counts_data)
        except Exception:
            pass
